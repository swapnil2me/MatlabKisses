...
assert(~isempty(strmatch(discovery_props.devicetype, {'HF2IS', 'HF2LI', 'UHFLI', 'UHFAWG', 'MFIA', 'MFLI'})), ...
       'Unknown device type.')

% Define default the default output mixer channel based on the device type and its options.
if strncmp(discovery_props.devicetype, 'UHF', 3) & ~ismember('MF', discovery_props.options)
  if output_channel == 0
    output_mixer_channel = '3';
  else
    output_mixer_channel = '7';
  end
elseif strncmp(discovery_props.devicetype, 'HF2LI', 5) & ~ismember('MFK', discovery_props.options)
  if output_channel == 0
