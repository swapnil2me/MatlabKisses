function ziAddPath
% ZIADDPATH add the LabOne Matlab drivers, utilities and examples to Matlab's Search Path
%
% USAGE:
% ziAddPath();
...
examplesPath = [pathstr filesep 'Examples'];
utilsPath = [pathstr filesep 'Utils'];
if isdir(driverPath) && isdir(examplesPath) && isdir(utilsPath)
    addpath(driverPath);
    addpath(genpath(examplesPath));
    addpath(utilsPath);
    dbs = dbstack;
    % If ziAddPath was called directly in Matlab output some help, otherwise output nothing.
    if strcmp(dbs(end).name, 'ziAddPath')
        fprintf('Added ziDAQ''s Driver, Utilities and Examples directories to Matlab''s path\n')
    ...
    fprintf('Error: ''Driver'', ''Examples'' and ''Utils'' are not subfolders of\n');
    fprintf('%s\n', pathstr);
