function hdawg_example_awg_sourcefile(device_id, varargin)
% HDAWG_EXAMPLE_AWG_SOURCEFILE demonstrates how to connect to a Zurich
% Instruments Arbitrary Waveform Generator and compile/upload an AWG
% program to the instrument.
%
% USAGE HDAWG_EXAMPLE_AWG_SOURCEFILE(DEVICE_ID)
% USAGE HDAWG_EXAMPLE_AWG_SOURCEFILE(DEVICE_ID, FILE_NAME)
%
...
if isempty(p.Results.awg_sourcefile)
    awg_sourcefile = 'ziDAQ_hdawg_example_awg_sourcefile.seqc';
...
ziDAQ('set', h, 'awgModule/compiler/sourcefile', awg_sourcefile);
ziDAQ('set', h, 'awgModule/compiler/start', 1);
...
if compilerStatus == 1
    ...
    while ziDAQ('getDouble', h, 'awgModule/progress') < 1
        fprintf('%d progress: %.1f%%.\n', k, ziDAQ('getDouble', h, 'awgModule/progress'));
        pause(0.1);
        k = k + 1;
    end
    fprintf('%d progress: %.1f%%.\n', k, 100*ziDAQ('getDouble', h, 'awgModule/progress'));
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
