function [data, config] = example_multidevice_sweep(device_ids, varargin)
...
for i = 1:length(device_ids)
    ziDAQ('connectDevice', device_ids{i}, '1GbE');
end
ziDAQ('sync');
...
devices = '';
for i = 1:length(device_ids)
    devices = strcat(devices, ',', device_ids{i});
end
...
if synchronize
    ...
    ziDAQ('set', multiDeviceSyncModule, 'multiDeviceSyncModule/start', 0);
    ziDAQ('set', multiDeviceSyncModule, 'multiDeviceSyncModule/group', 0);
    ziDAQ('execute', multiDeviceSyncModule);
    ziDAQ('set', multiDeviceSyncModule, 'multiDeviceSyncModule/devices', devices);
    ziDAQ('set', multiDeviceSyncModule, 'multiDeviceSyncModule/start', 1);

    timeout = 10;
    tstart = now();
    while true
        pause(0.2);
        status = ziDAQ('getInt', multiDeviceSyncModule, 'multiDeviceSyncModule/status');
        assert (status ~= -1, 'Error during device sync');
        if status == 2
            break
        end
        assert (now() - tstart < timeout, 'Timeout during device sync');
    end
end
...
ziDAQ('set', sweeper, 'sweep/device', device_ids{1});
...
ziDAQ('set', sweeper, 'sweep/bandwidthcontrol', 2);
...
% Disable, 1 = Enable.
ziDAQ('set', sweeper, 'sweep/bandwidthoverlap', 0);

% Sequential scanning mode (as opposed to binary or bidirectional).
ziDAQ('set', sweeper, 'sweep/scan', 0);
...
ziDAQ('set', sweeper, 'sweep/settling/time', 0);
...
ziDAQ('set', sweeper, 'sweep/settling/inaccuracy', 0.001);
% Set the minimum time to record and average data to 10 demodulator
% filter time constants.
ziDAQ('set', sweeper, 'sweep/averaging/tc', 10);
...
ziDAQ('set', sweeper, 'sweep/averaging/sample', 10);
