...
% e.g., averaging, on the recorded data (trigger/grid/operation is 0).
%
...
ziDAQ('set', h, 'dataAcquisitionModule/type', 1);
%   trigger/triggernode (char):
...
% Note: We do not manually set trigger/level and trigger/hysteresis in
% this example, rather we set the trigger/findlevel parameter to 1 and let
...
% ziDAQ('set', h, 'dataAcquisitionModule/level', 0.100);
% trigger/hysteresis (double):
...
% ziDAQ('set', h, 'dataAcquisitionModule/hysteresis', 0.005);

% The size of the internal buffer used to store data, this should be larger
% than trigger_duration.
% Unrequired parameters when trigger/type is EDGE_TRIGGER:
% ziDAQ('set', h, 'dataAcquisitionModule/bitmask', 1)  % For DIGITAL_TRIGGER
% ziDAQ('set', h, 'dataAcquisitionModule/bits', 1)  % For DIGITAL_TRIGGER
% ziDAQ('set', h, 'dataAcquisitionModule/bandwidth', 10);  % For TRACKING_TRIGGER

% Data Acquisition Module Grid Mode configuration:
% trigger/grid/mode (int)
...
% Note: trigger/grid/operation is not relevant if repetitions is 1, see
% below.
% trigger/grid/operation (int)
%   If the number of repetitions > 1, either replace or average the data in
%     the grid:
%     0: Replace.
%     1: Average.
% ziDAQ('set', h, 'dataAcquisitionModule/grid/operation', 1);
% trigger/grid/repetitions (int)
%   The number of times to perform trigger/grid/operation.
num_repetitions = 1;
ziDAQ('set', h, 'dataAcquisitionModule/grid/repetitions', num_repetitions);
% trigger/grid/cols (int)
...
ziDAQ('set', h, 'dataAcquisitionModule/delay', trigger_delay);
ziDAQ('set', h, 'dataAcquisitionModule/grid/cols', num_cols);
% Do not return overlapped trigger events.
ziDAQ('set', h, 'dataAcquisitionModule/holdoff/time', trigger_duration);
ziDAQ('set', h, 'dataAcquisitionModule/holdoff/count', 0);

% trigger/grid/rows (int)
%   Specify the number of rows in the grid's matrix. Each row is the data
%   recorded from one trigger.
num_rows = 500;
ziDAQ('set', h, 'dataAcquisitionModule/grid/rows', num_rows);
% trigger/grid/direction (int)
...
ziDAQ('set', h, 'dataAcquisitionModule/grid/direction', 0);

% The number of grids to record (if not running in endless mode). In grid
% mode, we will obtain trigger/count grids. The total number of total triggers
% that the module will record will be:
% n = trigger/count * trigger/grid/rows * trigger/grid/repetitions
ziDAQ('set', h, 'dataAcquisitionModule/count', num_grids);
...
ziDAQ('set', h, 'dataAcquisitionModule/enable', 1);
% Tell the Data Acquisition Module to determine the trigger level.
ziDAQ('set', h, 'dataAcquisitionModule/findlevel', 1);
findlevel = 1;
timeout = 10;  % [s]
t0 = tic;
while (findlevel == 1)
  pause(0.05);
  findlevel = ziDAQ('getInt', h, 'dataAcquisitionModule/findlevel');
  if toc(t0) > timeout
    ziDAQ('finish', h);
    ziDAQ('clear', h);
...
level = ziDAQ('getDouble', h, 'dataAcquisitionModule/level');
hysteresis = ziDAQ('getDouble', h, 'dataAcquisitionModule/hysteresis');
fprintf('Found and set dataAcquisitionModule/level: %.3e, dataAcquisitionModule/hysteresis: %.3e\n', level, hysteresis);
...
while ~ziDAQ('finished', h)
    ...
    if ~ziCheckPathInData(data_read, triggernode_us)
      ...
      % If trigger/count > 1 then more than one grid could be returned.
