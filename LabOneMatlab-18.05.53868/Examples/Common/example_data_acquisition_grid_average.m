...
% enabled when dataAcquisitionModule/grid/repetitions is > 1 and
% dataAcquisitionModule/grid/operation is set to average. The Data
...
ziDAQ('set', h, 'dataAcquisitionModule/level', 0.04);
...
% dataAcquisitionModule/grid/mode (int)
...
%   The number of times to perform dataAcquisitionModule/grid/operation.
num_repetitions = 30;
...
ziDAQ('set', h, 'dataAcquisitionModule/duration', trigger_duration);
...
% mode, we will obtain dataAcquisitionModule/count grids. The total number of total triggers
% that the module will record will be:
% n = dataAcquisitionModule/count * dataAcquisitionModule/grid/rows * dataAcquisitionModule/grid/repetitions
num_grids = 1;
...
ziDAQ('execute', h);
pause(1.2*ziDAQ('getDouble', h, 'dataAcquisitionModule/buffersize'));
...
while ~ziDAQ('finished', h)
    ...
    if ~ziCheckPathInData(data, triggernode_us);
      ...
      % If dataAcquisitionModule/count > 1 then more than one grid could be returned.
      ...
      for n=1:num_grids
        ...
        if bitand(flags, 1)
          ...
          % We could save the finished grid data here (if dataAcquisitionModule/count > 1 and we
...
if ~bitand(flags, 1)
  ...
  data = ziDAQ('read', h);
  % Here dataAcquisitionModule/count is 1: Only one grid in data.
end
