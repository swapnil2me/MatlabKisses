function [data, config] = example_swtrigger_grid(device_id, varargin)
% EXAMPLE_SWTRIGGER_GRID Record data using a demodulator trigger via ziDAQ's record module
%
% USAGE DATA = EXAMPLE_SWTRIGGER_GRID(DEVICE_ID)
%
% Record demodulator sample data using a Software Edge Trigger in
...
% rows of the grid. This example demonstrates basic Grid Mode usage without an
% addition operation, e.g., averaging, on the recorded data
% (trigger/0/grid/operation is 0).
%
% This example records the demodulator data as is - essentially a constant
% value with noise. The SW Trigger's 'find' functionality calculates an
...
% The number of SW Trigger grids to record.
...
% ziDAQ('set', h, 'trigger/0/level', 0.100);
...
data.(device).demods.sample = {};

%% Start recording
% Start the module's thread -> ready to be triggered.
ziDAQ('execute', h);
% Tell the SW Trigger to determine the trigger level.
ziDAQ('set', h, 'trigger/0/findlevel', 1);
findlevel = 1;
timeout = 10;  % [s]
t0 = tic;
while (findlevel == 1)
  pause(0.05);
  findlevel = ziDAQ('getInt', h, 'trigger/0/findlevel');
  if toc(t0) > timeout
    ziDAQ('finish', h);
    ziDAQ('clear', h);
    error('SW Trigger didn''t find trigger level after %.3f seconds.\n', timeout)
  end
end
level = ziDAQ('getDouble', h, 'trigger/0/level');
hysteresis = ziDAQ('getDouble', h, 'trigger/0/hysteresis');
fprintf('SW Trigger found and set trigger/0/level: %.3e, trigger/0/hysteresis: %.3e\n', level, hysteresis);
...
while ~ziDAQ('finished', h)
    ...
    if ~ziCheckPathInData(data_read, triggerpath)
      fprintf('No update available since last read.\n');
    else
      % If trigger/0/count > 1 then more than one grid could be returned.
      num_grids_read = length(data_read.(device).demods(trigger_demod_index).sample);
      for n=1:num_grids_read
        flags = data_read.(device).demods(trigger_demod_index).sample{n}.header.flags;
        if bitand(flags, 1)
          ...
          data.(device).demods.sample{end+1} = data_read.(device).demods(trigger_demod_index).sample{n};
      ...
      % grid.
      R = abs(data_read.(device).demods(trigger_demod_index).sample{end}.x + ...
              1j*data_read.(device).demods(trigger_demod_index).sample{end}.y);
      maxR = max(R(:));
      minR = min(R(:));
      if minR < minZ; minZ = minR; end
      if maxR > maxZ; maxZ = maxR; end
      update_plot(plot_handle, R , data_read.(device).demods(trigger_demod_index).sample{end}.timestamp, minZ, maxZ);
    end
    if toc(t0) > timeout
        ...
        if num_finished_grids == 0
            ...
            fprintf('Loop timed-out (%.2f s) before acquiring %d triggers\n', timeout, trigger_count);
...
if ~bitand(flags, 1)
  ...
  fprintf('SW Trigger finished since last intermediate read() in loop, reading out finished grid(s).\n');
  data_read = ziDAQ('read', h);
  num_grids = length(data_read.(device).demods(trigger_demod_index).sample);
  for n=1:num_grids
    flags = data_read.(device).demods(trigger_demod_index).sample{n}.header.flags;
    if flags & 1
      data.(device).demods.sample{end+1} = data_read.(device).demods(trigger_demod_index).sample{n};
...
assert(ziCheckPathInData(data, triggerpath), 'Ooops, read() didn''t return the subscribed data - misconfig?');
