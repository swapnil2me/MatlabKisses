...
ziDAQ('set', advisor, 'pidAdvisor/pid/mode', 3)
...
if dut_source == 4
    ...
    ziDAQ('set', advisor, 'pidAdvisor/pid/type', 'pll');
    % The PID Advisor is appropriate for optimizing the HF2's PLL
    % parameters (pidAdvisor/pid/type set to 'pll') or the HF2's PID
    % parameters (pidAdvisor/pid/type set to 'pid').
end
...
while calculate == 1
  pause(0.5)
  calculate = ziDAQ('getInt', advisor, 'pidAdvisor/calculate');
  if toc > timeout
...
if ~isempty(result)
  ...
  ziDAQ('set', advisor, 'pidAdvisor/todevice', 1)
