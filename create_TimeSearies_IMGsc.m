function create_TimeSearies_IMGsc(locn,flnm,bkg,ap,sweep)
cd(locn)
direct = 'Mat_file';
...
if strcmp(sweep,'vgdc')
    fig;imagesc(datenum([data.timeStamp]),[data.FreqF],(ampF-bkgD))
    title('Freq v/s Time')
...
if strcmp(sweep,'vgdc')
    Plot_Prop(fig,'Time','lin','Freq (MHz)','lin','Time series')
elseif strcmp(sweep,'vgac')
    Plot_Prop(fig,'Time','lin','Freq (MHz)','lin','Time series')
elseif strcmp(sweep,'vsdac')
    Plot_Prop(fig,'Time','lin','Freq (MHz)','lin','Time series')
